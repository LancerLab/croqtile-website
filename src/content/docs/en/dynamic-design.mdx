...
## Overview
In this section, you will learn about the *anonymous dimension* and *symbolic dimension* in CroqTile and how they support dynamic shapes required by certain systems.
...
CroqTile supports shape dimensions with unknown values, similar to many high-level machine learning languages. The simplest method is to use **Anonymous Dimension** in `mdspan`:
...
Another approach in CroqTile to support dynamic shape is to use **Symbolic Dimension**. *Symbolic dimension*s are named but as well unknown at compile-time. Here's an example:
...
The reason for improved code safety of using *symbolic dimension* is that more comprehensive code checks can be applied by CroqTile compiler. For example, consider an *anonymous dimension* version of the `Matmul` function:
...
CroqTile performs both compile-time and runtime checks to ensure safety:

- **Compile-time Checks**: CroqTile verifies rank consistency. In this example, `lhs.span` has a ranked of `2`, matching the rank of `{tile_m, tile_k}`, so there are no issues.
- **Runtime Checks**: CroqTile generates code to validate dimensions at runtime. For instance, in the declaration of `tiled_lhs`, its shape must not have a dimension of `0`, which would result in an invalid zero-sized buffer. CroqTile's *transpilation* process generates the following *target host code*:
...
Here, the `croqtile_abort` function aborts program execution if the condition is not met. This early check helps CroqTile detect issues promptly.

For the *symbolic dimension* version, CroqTile compiler generates additional checks in the *target host code*:
...
## Quick Summary
IIn this section, we discussed *anonymous dimension* and *symbolic dimension* in CroqTile, which support dynamic shapes required in certain scenarios. Compared to *anonymous dimension*, using *symbolic dimension* enhance code safety through additional compile-time checks, making them the recommended choice for ease of use and improved safety.
